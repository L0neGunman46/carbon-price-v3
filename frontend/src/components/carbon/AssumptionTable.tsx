...
export default function AssumptionTable({
  marketData, draftPrices, onPriceChange, onSubmitSuccess,
}: Props) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-base font-semibold text-gray-900">Your Price Assumptions</h2>
      ...
      <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
        <span className="text-xs text-gray-400 flex items-center gap-1.5">
          {isAdmin ? (
            <><CheckCircle2 className="w-3.5 h-3.5 text-green-500" /> Saves activate immediately (Admin)</>
          ) : (
            <><Clock className="w-3.5 h-3.5 text-amber-500" /> Changes require admin approval</>
          )}
        </span>
        <div className="flex items-center gap-3">
          {toast && (
            <span className={`text-sm ${toast.ok ? 'text-green-600' : 'text-red-500'}`}>
              {toast.msg}
            </span>
          )}
          <Button
